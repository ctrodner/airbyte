...
class CdcColdStartWorker(override val input: CdcNotStarted) : CdcWorker<CdcNotStarted>() {
    ...
    override fun call(): WorkResult<GlobalKey, CdcNotStarted> {
        /**
        ...
        return WorkResult(input, input.completed(fakeGlobalStateValue))
    }
}
...
class CdcOngoingWorker(override val input: CdcOngoing) : CdcWorker<CdcOngoing>() {
    ...
    override fun call(): WorkResult<GlobalKey, CdcOngoing> {
        /**
         ...
         */
        return WorkResult(input, input.ongoing(input.checkpoint).completed())
    }
}
